...
class ParkingState extends ChamberState
{
	...
	override public function create():Void
	{
		...
		avatarSleeping.animation.frameIndex = 0;
		...
		otherSleeping.animation.frameIndex = 0;
		...
		else 
		{
			...
			startedAsleep = true;
			FlxG.camera.fade(0xFF000000,20,true);
		}
	...
	{
		if (avatarSleeping.visible && avatarSleeping.animation.frameIndex == 0 && FlxG.keyboard.justPressed("SPACE"))
		{
			avatarSleeping.animation.frameIndex = 1;
			help.setText(GameAssets.PARKING_GET_UP_INSTRUCTION);
		}
		else if (
			avatarSleeping.visible == true && 
			avatarSleeping.animation.frameIndex == 1 && 
			FlxG.keyboard.justPressed("ENTER"))
		{
			avatarSleeping.visible = false;
			avatar.sprite.visible = true;
