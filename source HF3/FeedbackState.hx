...
class FeedbackState extends ChamberState
{
	...
	override public function handleInput():Bool
	{
		...
		{
			if (!avatar.sprite.visible && FlxG.keyboard.justPressed("ENTER"))
			{
				desk.sprite.animation.frameIndex = 2;
	...
	override public function handleMessageInput():Bool
	{
		...
		else if (state == CONFIRMING)
		{
			if (FlxG.keyboard.justPressed("ENTER"))
			{
				...
				timer = FlxTimer.start(1,showThanksMessage);
				help.fadeOut();
			}
	...
	override public function handleTriggers():Bool
	{
		...
		if (avatarMovementEnabled && avatar.overlaps(deskTrigger))
		{
			...
			if (FlxG.keyboard.justPressed("ENTER"))
			{
				...
				desk.sprite.animation.frameIndex = 1;

				timer = FlxTimer.start(1,showFeedbackInstructions);
			}
		}
