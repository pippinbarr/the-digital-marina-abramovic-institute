...
class EntranceState extends FlxState
{
	...
	override public function create():Void
	{
		...
		doors.y = 276;
		doors.animation.add("open",[0,1,2,3,4,5],10,false);
		doors.animation.add("close",[5,4,3,2,1,0],10,false);
		doors.origin.y = 1.0;
	...
	private function handleCollisions():Void
	{
		...
		if (doors.animation.frameIndex == 0 && !movementEnabled)
		{
			closing = false;
		...
		{
			if (doors.animation.frameIndex == 0 && avatar.y >= FlxG.height - 100)
			{
				doors.animation.play("open");
			}
		}
		else if (doors.animation.frameIndex == 5)
		{
