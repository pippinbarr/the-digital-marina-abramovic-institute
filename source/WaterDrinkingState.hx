...
class WaterDrinkingState extends ChamberState
{
	...
	override public function create():Void
	{
		...
		drinkingAvatar.addAnimation("raise_full",[0,1,2,3,4],2,false);
		drinkingAvatar.addAnimation("lower_full",LOWER_FULL_FRAMES,2,false);
		drinkingAvatar.addAnimation("raise_empty",[5,6,7,8,9],2,false);
		drinkingAvatar.addAnimation("lower_empty",LOWER_EMPTY_FRAMES,2,false);

		Helpers.scaleSprite(drinkingAvatar);
		drinkingAvatar.frame = 0;
	...
	override public function handleInput():Bool
	{
		...
		if (currentWater != NO_WATER && !drinking && !raising && !lowering && FlxG.keys.ENTER)
		{
			...
			{
				drinkingAvatar.play("raise_full");
			}
			else
			{
				drinkingAvatar.play("raise_empty");
			}
		...
		{
			if (drinkingAvatar.frame == 4 || drinkingAvatar.frame == 9)
			{
				raising = false;
				drinking = true;
				showWaterMeter();
		...
		else if (drinking && !FlxG.keys.ENTER)
		{
			if (waterRemaining[currentWater] > 0)
			{
				...
				// drinkingAvatar.frame = drinkingAvatar.frame;
				drinkingAvatar.play("lower_full");
			}
			else
			{
				drinkingAvatar.play("lower_empty");
		...
		{
			if (drinkingAvatar.frame == 0 || drinkingAvatar.frame == 5)
			{
				if (waterRemaining[currentWater] <= 0) 
				{
		...
		if (avatar.overlaps(trigger))
		{
			if (!glass.visible && !drinking && !lowering && !raising)
			{
				...
				if (FlxG.keys.justPressed("SPACE"))
				{
					glass.visible = true;
					avatar.switchToCoat();
					if (waterRemaining[currentWater] == 0) glass.frame = 3;
					else if (waterRemaining[currentWater] < 0.33) glass.frame = 2;
					else if (waterRemaining[currentWater] < 0.66) glass.frame = 1;
					else glass.frame = 0;
					currentWater = NO_WATER;
					avatar.sprite.frame = 29;
				}
			}
