...
class LuminosityState extends ChamberState
{
	...
	override public function update():Void
	{
			...
			Math.abs(inCount - outCount)/Math.max(inCount,outCount) <= MAX_BREATH_DIFFERENCE)
		{
			...
			inCount = 0;
			outCount = 0;
			timer.start(12,1,checkLevitationBreathing);
	...
	override public function handleInput():Bool
	{
		...
		if (avatarMovementEnabled && avatar.overlaps(bedTrigger) && FlxG.keys.justPressed("ENTER"))
		{
		...
		else if (bedAvatar.visible && bedAvatar.frame == 0 && !levitating && FlxG.keys.justPressed("SPACE"))
		{
		...
		else if (levitating && !stoppedLevitating && FlxG.keys.justPressed("SPACE"))
		{
		...
		else if (bedAvatar.visible && bedAvatar.frame == 1 && FlxG.keys.justPressed("SPACE"))
		{
		...
		else if (!avatar.sprite.visible && bedAvatar.frame == 0 && FlxG.keys.justPressed("ENTER"))
		{
		...
		if (bedAvatar.visible && bedAvatar.frame == 1 && FlxG.keys.UP)
		{
			if (breathing == OUT)
			{
				breathing = IN;
		...
		else if (bedAvatar.visible && bedAvatar.frame == 1 && FlxG.keys.DOWN)
		{
			if (breathing == IN)
			{
				breathing = OUT;
		...
		if (bedAvatar.visible && bedAvatar.frame == 1 && levitating)
		{
			help.setText(GameAssets.LUMINOSITY_BREATHE_INSTRUCTION + "\n" + GameAssets.LUMINOSITY_STOP_LEVITATING_INSTRUCTION);
		}
		else if (bedAvatar.visible && bedAvatar.frame == 1 && !levitating)
		{
			help.setText(GameAssets.LUMINOSITY_BREATHE_INSTRUCTION + "\n" + GameAssets.OPEN_EYES_INSTRUCTION);			
		}
	...
	{
		if (!Helpers.sleepKeyIsDown && bedAvatar.visible && bedAvatar.frame == 1)
		{
			bedSleeping = true;
			breathMeter.visible = false;
